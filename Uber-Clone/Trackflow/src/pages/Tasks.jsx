...
export default function Tasks() {
  ...
  useEffect(() => { setPage(1); }, [searchQuery, filterStatus, filterPriority, filterType]);
  ...
  const filtered = tasks
    .filter((t) => {
      ...
      const matchSearch   = !q || t.title.toLowerCase().includes(q) || t.id.toLowerCase().includes(q) || t.tags?.some((tag) => tag.toLowerCase().includes(q)) || t.description?.toLowerCase().includes(q);
  ...
  const getSprint = (id) => sprints.find((s) => s.id === id);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl px-5 py-3.5 shadow-sm border border-gray-100 flex items-center gap-3 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {hasActiveFilters && (
            <button onClick={clearFilters} className="text-xs text-indigo-500 hover:text-indigo-700 font-medium">
              Clear all
            </button>
          )}
        </div>
